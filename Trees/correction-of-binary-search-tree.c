...
//Start by straightening out the entire BST
//So that can traverse like a linked list
//then locate the 2 faulty nodes x and y
while(head != NULL)
{
    if(head->left != NULL)
    {
        ...
        if(cur->right != NULL)
        {
            if(prev != NULL && prev->item > head->item)
            {
                if(x == NULL)
                {
                    x = prev;
...
//perform the swap of x and y in the end
if(x != NULL && y != NULL)
{
    temp = x->item;
    x->item = y->item;
