...
int HashDelete(HashTable* Q3HashPtr, int key)
{
    ...
    if(Q3HashPtr->nSize == 0) return 0;

    int index = Hash(key, Q3HashPtr->hSize);
    ListNode *target = HashSearch(*Q3HashPtr, key);
    ...
    while(((cur->next)->key) != key) cur = cur->next;
    if(target->next == NULL) cur->next = NULL;
    else cur->next = target->next;
    free(target);
    return 1;
}
